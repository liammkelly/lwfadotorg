...
<cfparam name="totalAverages" default="0">
...
<cfif qBox.week eq application.currwk>
    <cfquery name="qAway" datasource="lwfa">
        select 	p.*,s.opp,t.name "team",l.total,l.line, 1 as live
        from 	players p join nflschedule s on p.nflteam=s.NFLTM  join teams t on p.teamID=t.teamID left join v_current_stats l on p.playerID=l.playerID
...
<cfelseif qBox.week gt application.currwk>
    <cfquery name="qAway" datasource="lwfa">
        select 	p.*,concat('OPPONENT: ',s.opp) "line",s.opp,ps.seasonPPG "total",t.name "team", 0 as live
...
<cfelse>
    <cfquery name="qAway" datasource="lwfa">
        select 	p.playerID,p.first,p.last,p.pos,p.view,p.nflteam,'#qBox.awayTeamID#' as teamID,pd.line,pd.total,'' as opp , 0 as live
    ...
    <cfquery name="qHome" datasource="lwfa">
        select 	p.playerID,p.first,p.last,p.pos,p.view,p.nflteam,'#qBox.homeTeamID#' as teamID,pd.line,pd.total,'' as opp, 0 as live
        from 	players p join playerdata pd on p.playerID=pd.playerID join teams t   on pd.teamID=t.teamID
        where 	t.teamID=#qBox.homeTeamID# AND
                	pd.weekNo=#qBox.week# AND
                	pd.active='Y'
...
<cfloop list="home,away" index="l">
	<cfloop list="QB,RB,FX,WR1,WR2,TE,K,D" index="p">
		...
		<cfset thisGame[l&"Team"][p]['image'] = '/2010/images/players/unknown.jpg'>
...
<cfloop query="qHome">
	...
	<cfelse>
	    ...
	    <cfif qStats.recordcount gt 0 AND qBox.week lte application.currwk>
		...
		<cfelse>
			...
			<cfif isNumeric(qStats.seasonPPG) AND totalAverages AND dayofweek(now()) gt 2>
		    	<cfset variables.total=qStats.seasonPPG>
			<cfelse>
    ...
    <cfset thisGame.homeTeam.total=thisGame.homeTeam.total+variables.total>
   	<cfset variables.imageView="_images/players/"&playerid&".jpg">
	<cfif pos eq 'QB'>
	...
	<cfelseif pos eq 'TE'>
    	<cfif structKeyExists(thisGame.homeTeam.TE,'first') AND thisGame.homeTeam.TE.first neq ''>
			<cfset thisGame.homeTeam.FX.first=left(first,1)&". ">
			<cfset thisGame.homeTeam.FX.last=last>
			<cfset thisGame.homeTeam.FX.nflteam=nflteam>
			<cfset thisGame.homeTeam.FX.opp=opp>
...
<cfloop query="qAway">
	...
	<cfelse>
	    ...
	    <cfif qStats.recordcount AND qBox.week lte application.currwk>
			<cfset variables.line=qStats.line>
	    	<cfset variables.total=qStats.total>
		<cfelse>
			<cfquery name="qStats" datasource="lwfa">
	...
	<cfelseif pos eq 'TE'>
    	<cfif structKeyExists(thisGame.awayTeam.TE,'first') AND thisGame.awayTeam.TE.first neq ''>
			<cfset thisGame.awayTeam.FX.first=left(first,1)&". ">
			<cfset thisGame.awayTeam.FX.last=last>
			<cfset thisGame.awayTeam.FX.nflteam=nflteam>
			<cfset thisGame.awayTeam.FX.opp=opp>
...
<cfif dayofweek(now()) eq 1>
	...
	<center>Last update: <cfoutput>#datediff('n',qQuery.updateTime,now())#:#numberformat(datediff('s',qQuery.updateTime,now()) mod 60,'00')#</cfoutput> ago<br /></center>
</cfif>
...
<table cellpadding="0" cellspacing="0"<cfif isMobile eq 0> class="thisBoxTbl"</cfif>>
	...
	<cfelse>
		<tr>
	    	<td colspan=7 class="header" align="center">#thisGame.awayTeam.team# #thisGame.awayTeam.nickname# #thisGame.awayTeam.record# AT #thisGame.homeTeam.team# #thisGame.homeTeam.nickname# #thisGame.homeTeam.record# <img src="images/icon_post.png" onclick="postComment('game',#gameID#,'the Week #qBox.week# #thisGame.awayTeam.nickname#/#thisGame.homeTeam.nickname# matchup')"></td>
			<!---  
